...
export default class AuthController {
    ...
    static async SignUpOtpVerification(req: Request, res: Response): Promise<Response> {
        try {
            ...
            let userData: any = await redisClient.get(`sign_up_verification_session:${authSessionSchema.parse(req.cookies.sign_up_verification_session)}`);
            if (!userData) {
                return res.status(400).json({ data: null, error: { message: 'Signup sesssion has expired' }, success: false });
            }
            userData = JSON.parse(userData);
            if (userData.otp !== req.body.otp) return res.status(400).json({ success: false, error: { message: "incorect otp" }, data: null });
            let user = await db
                .insert(usersTable)
