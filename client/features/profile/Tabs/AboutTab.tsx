...
export default function AboutTab({ isDefaultUserId, education, job, languages, about, religion }: IProps) {
  return (
          ...
          <CardContent className=" flex flex-col gap-y-2">
            {!!education && education.toReversed().map(({ degree, institution, startYear, endYear }, key) =>
              <div key={key} className="flex flex-row justify-between items-start">
                ...
                onClick={() => alert('This feature is in construction')}
                >&times;</button>
              </div>
            )}
          </CardContent>
          {!!education && education.length < 4 && isDefaultUserId &&
            <CardFooter >
              <CardAction>
                <AddEducationDialog />
              </CardAction>
